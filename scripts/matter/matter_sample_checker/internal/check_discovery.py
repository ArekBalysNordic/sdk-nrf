...
class CheckDiscovery:
    ...
    def discover_checks(self) -> List[Type[MatterSampleTestCase]]:
        ...
        python_files = [
            f for f in self.checks_directory.glob('*.py')
        ...
        for python_file in python_files:
            ...
            try:
                ...
                for name, obj in inspect.getmembers(module, inspect.isclass):
                    # Check if it's a subclass of MatterSampleTestCase (but not the base class itself)
                    if (issubclass(obj, MatterSampleTestCase) and
                            obj is not MatterSampleTestCase and
                ...
                print(
                    f"Warning: Could not import check from {python_file.name}: {e}")
    ...
    def discover_doc_checks(self, subdirectory: str = 'docs') -> List[Type[MatterSampleTestCase]]:
        ...
        python_files = [
            f for f in docs_directory.glob('*.py')
            if f.is_file() and f.name != '__init__.py'
        ]
        ...
        for python_file in python_files:
            ...
            try:
                ...
                module = importlib.import_module(
                    f'checks.{subdirectory}.{module_name}')
                ...
                # Log import errors but continue with other checks
                print(
                    f"Warning: Could not import doc check from {subdirectory}/{python_file.name}: {e}")
