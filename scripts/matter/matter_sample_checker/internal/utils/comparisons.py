...
def compare_overlay_files(current_file: Path, template_file: Path, config: Dict[str, Any]) -> List[str]:
        ...
        if line.strip().startswith(('*', '//', '/*', '#')):
            ...
            if stripped_line in ['/*', '*/', '*']:
                ...
                if years_in_line and all(2020 <= int(year) <= 2030 for year in years_in_line):
                    ...
                    if len(words_without_years) < 20 and any(word in template_line_lower for word in ['nordic', 'copyright', '(c)']):
                        return True
...
def compare_config_files(current_file: Path, template_file: Path, config: Dict[str, Any]) -> List[str]:
    ...
    for line in diff_lines:
        ...
        if not line.startswith(('+', '-')) or line.startswith(('+++', '---')):
            ...
            if not is_copyright_line:
                is_copyright_line = any(keyword in line_content_lower for keyword in copyright_keywords)
            ...
            if not is_copyright_line and (line_content.startswith(('*', '//', '/*', '#'))):
                ...
                if years_in_line and all(2020 <= int(year) <= 2030 for year in years_in_line):
                    # If the line is mostly just years and common words, it's likely a copyright year change
                    words_without_years = re.sub(year_pattern, '', line_content).strip()
                    if len(words_without_years) < 20 and any(word in line_content_lower for word in ['nordic', 'copyright', '(c)']):
    ...
    change_lines = [line for line in filtered_diff_lines if line.startswith(('+', '-')) and not line.startswith(('+++', '---'))]
    if not change_lines:
        return []
