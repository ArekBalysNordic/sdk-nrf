...
def find_config_files(directory: Path, config: Dict[str, Any]) -> List[Path]:
    """Find all configuration files (based on configured extensions) in the given directory recursively, excluding build directories and sample-specific files."""
    config_files = []
...
def find_template_directory(sample_path: Path) -> Path:
...
def parse_samples_zap_yaml(yaml_path, nrf_base=None):
    ...
    if not isinstance(data, list):
        print(f"Error: YAML file should contain a list of sample entries")
        sys.exit(1)
    ...
    for entry in data:
        ...
        if 'zap_file' in entry:
            ...
            # e.g., "samples/matter/light_bulb/src/default_zap/light_bulb.zap" -> "samples/matter/light_bulb"
            # or "applications/matter_bridge/src/default_zap/bridge.zap" -> "applications/matter_bridge"
            ...
            if 'src' in parts:
                ...
                if sample_path not in samples:
                    if sample_path.exists() and sample_path.is_dir():
                        ...
                        print(
                            f"Warning: Sample path does not exist or is not a directory: {sample_path}")
...
def discover_matter_samples(sample_path: Path) -> List[str]:
    ...
    for item in matter_dir.iterdir():
        if item.is_dir() and item.name != 'common' and not item.name.startswith('.'):
            # Check if it looks like a sample (has sample.yaml or CMakeLists.txt)
            if (item / 'sample.yaml').exists() or (item / 'CMakeLists.txt').exists():
...
def find_line_comment_issues(content: str) -> List[tuple]:
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        while i < len(line):
            ...
            if not in_block_comment and not in_string and i < len(line) - 1 and line[i:i+2] == '//':
...
def has_internal_build_configs(sample_yaml_path: Path) -> bool:
    ...
    try:
        ...
        print(
            f"Error checking internal build configs at {sample_yaml_path}: {e}")
        return False
        ...
        if line.startswith('+'):
            ...
            if change_count <= 10:  # Show more changes in verbose mode
                colored_line = colorize(
                    line.rstrip(), COLORS["green"])
                context_lines.append(f"    {colored_line}")
        elif line.startswith('-'):
            change_count += 1
            if change_count <= 10:  # Show more changes in verbose mode
                colored_line = colorize(
                    line.rstrip(), COLORS["red"])
                context_lines.append(f"    {colored_line}")
        elif line.startswith(' '):
...
def get_latest_matter_sha(nrf_root: Path) -> str:
    ...
    try:
        ...
        result = subprocess.run(['git', 'log', '--merges', '--oneline', '-1'],
                                cwd=matter_root, capture_output=True, text=True)
        if result.returncode == 0 and result.stdout.strip():
            merge_sha = result.stdout.split()[0]
            # Get the short version of the SHA
            short_result = subprocess.run(['git', 'rev-parse', '--short', merge_sha],
                                            cwd=matter_root, capture_output=True, text=True)
        ...
        result = subprocess.run(['git', 'log', '--grep=Merge', '--oneline', '-1'],
                                cwd=matter_root, capture_output=True, text=True)
        if result.returncode == 0 and result.stdout.strip():
            merge_sha = result.stdout.split()[0]
        ...
        result = subprocess.run(['git', 'log', '--grep=sync', '-i', '--oneline', '-1'],
                                cwd=matter_root, capture_output=True, text=True)
        if result.returncode == 0 and result.stdout.strip():
...
def get_matter_sha_link(nrf_root: Path, sha: str = None) -> str:
    ...
    try:
        ...
        full_sha_result = subprocess.run(['git', 'rev-parse', sha],
                                        cwd=matter_root, capture_output=True, text=True)
