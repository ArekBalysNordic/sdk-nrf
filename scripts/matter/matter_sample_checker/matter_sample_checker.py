...
from internal.utils.utils import parse_samples_zap_yaml, load_config
from internal.check_discovery import CheckDiscovery


def main():
    parser = argparse.ArgumentParser(
        description='Check Matter sample consistency in nRF Connect SDK',
...
Examples:
  ...
  python matter_sample_checker.py --allow-names "light bulb" template  # Allow specific names in copy-paste checking
  python matter_sample_checker.py -a "light bulb" "door lock" -v  # Allow multiple names with verbose output
  python matter_sample_checker.py --samples-zap-yaml zap_samples.yml --base /path/to/ncs/nrf  # Check multiple samples from YAML
  python matter_sample_checker.py -s zap_samples.yml -b /path/to/ncs/nrf -y 2024  # Combined with year check
        """
    )
    parser.add_argument('sample_path', nargs='?', default='.',
                        help='Path to the Matter sample directory to check (default: current directory). Ignored if --samples-yaml is used.')
    parser.add_argument('--samples-zap-yaml', '-s', type=str,
                        help='Path to YAML file containing list of samples to check (similar to zap_samples.yml format)')
    parser.add_argument('--base', '-n', type=str,
                        help='Base directory for resolving sdk-nrf path. If not specified, paths are resolved using ZEPHYR_BASE/../nrf')
    parser.add_argument('--verbose', '-v', action='store_true',
                        help='Show verbose output during checks')
    parser.add_argument('--year', '-y', type=int, nargs='*',
                        help='Enable copyright year checking. If no years specified, checks current year. Can specify multiple years (e.g., --year 2023 2024 2025). Default: skip year checking')
    parser.add_argument('--config', '-c', type=str,
                        help='Path to custom configuration YAML file. Default: matter_sample_checker_config.yaml in script directory')
    parser.add_argument('--allow-names', '-a', type=str, nargs='*', default=[],
                        help='List of names/terms to allow during copy-paste error checking (case-insensitive). Use quotes for multi-word names.')
    ...
    config_path = args.config if args.config else Path(
        __file__).parent / 'matter_sample_checker_config.yaml'
    ...
    for sample_path in sample_paths:
        checker = MatterSampleChecker(
            config_dict, nrf_base, sample_path,
            verbose=args.verbose,
            allowed_names=args.allow_names,
            expected_years=expected_years,
