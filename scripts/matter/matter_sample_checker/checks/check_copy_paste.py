...
class CopyPasteTestCase(MatterSampleTestCase):
    ...
    def check(self):
                        ...
                        if re.search(word_pattern, content, re.IGNORECASE):
                            ...
                            for i, line in enumerate(lines, 1):
                                if re.search(word_pattern, line, re.IGNORECASE):
                                    ...
                                    # Skip defined patterns - skip completely, don't log at all
                                    ...
                                    if is_readme:
                                        # For README files, log as DEBUG and increment counter
                                        self.debug(
                                        ...
                                        # Non-README files: treat as error (likely copy-paste mistake)
                                        self.issue(
        ...
        if self.readme_issues_count > 0:
            ...
            self.info(
                f"Found {self.readme_issues_count} potential copy-paste mistake(s) in the README.rst file. There are references to the following words:")
            for word in found_words_list:
                self.info(f"  - {word}")
            self.info(
                f"Run the script again with --verbose to see details.")
