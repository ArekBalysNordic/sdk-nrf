...
class SampleYamlTestCase(MatterSampleTestCase):
    ...
    def check(self):
        ...
        sample_name_pattern = self.config.config_file.get(
            'sample_yaml').get('sample_name_pattern')
        ...
        if sample_yaml_path.exists():
            try:
                ...
                for test_name in sample_data.get('tests'):
                    # Test name is in convenction sample.matter.<sample_name>.variant
                    if not re.match(sample_name_pattern, str(test_name)):
                        self.issue(
                            f"Sample name in sample.yaml does not match directory name: {test_name}")

                # Check for ci_samples_matter tag
                tags = sample_data.get('common').get('tags')
                if not any(tag in tags for tag in required_tags):
                    self.issue(
                        f"Missing required tag in sample.yaml: at least one of {required_tags} must be present in {tags}")
