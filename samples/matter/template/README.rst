...
An additional partition for MCUboot is created in the external memory and it is used to perform a firmware update of the Wi-Fi firmware.

To disable this feature and keep the Wi-Fi firmware in the on-chip memory, perform the following actions:

   * Set the ``SB_CONFIG_WIFI_PATCHES_EXT_FLASH_STORE`` Kconfig option to ``n``.
   * Change the ``nrf70_wifi_fw`` partition locations in the associated :file:`pm_static_*.yml` file.

To learn how to configure MCUboot partitions, see the :ref:`nrf70_fw_patch_update_adding_partitions` guide.

.. important::

   This feature does not work with tools from the `nRF Command Line Tools`_ product page and the nrfjprog programming tool.
