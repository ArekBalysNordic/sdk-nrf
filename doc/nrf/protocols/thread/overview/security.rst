...
By default, the OpenThread stack uses the :ref:`nrf_security` (nrf_security) and `Platform Security Architecture (PSA)`_ for cryptographic operations.
...
The end slot number is calculated as a sum of the start slot number and the maximum number of keys that can be stored in the PSA ITS defined in the :kconfig:option:`CONFIG_OPENTHREAD_PSA_ITS_NVM_MAX_KEYS` Kconfig option.
