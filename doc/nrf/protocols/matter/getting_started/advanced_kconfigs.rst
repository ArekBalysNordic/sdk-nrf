...
To enable the Matter Settings shell module, set the :ref:`CONFIG_NCS_MATTER_SETTINGS_SHELL<CONFIG_NCS_MATTER_SETTINGS_SHELL>` Kconfig option to ``y``.
...
|NCS| Matter applications use one generic Persistent Storage API that can be enabled by the :ref:`CONFIG_NCS_MATTER_PERSISTENT_STORAGE<CONFIG_NCS_MATTER_PERSISTENT_STORAGE>` Kconfig option.
...
* :ref:`CONFIG_NCS_MATTER_SETTINGS_STORAGE_BACKEND<CONFIG_NCS_MATTER_SETTINGS_STORAGE_BACKEND>` - Activates the implementation that takes advantage of the raw :ref:`Zephyr settings<zephyr:settings_api>`.
...
* :ref:`CONFIG_NCS_MATTER_SECURE_STORAGE_BACKEND<CONFIG_NCS_MATTER_SECURE_STORAGE_BACKEND>` - Activates the module based on the ARM PSA Protected Storage API implementation from the :ref:`trusted_storage_readme` |NCS| library.
...
You can do this using the :ref:`CONFIG_NCS_MATTER_STORAGE_MAX_KEY_LEN<CONFIG_NCS_MATTER_STORAGE_MAX_KEY_LEN>` Kconfig option.

If both backends are activated at the same time (:ref:`CONFIG_NCS_MATTER_SETTINGS_STORAGE_BACKEND<CONFIG_NCS_MATTER_SETTINGS_STORAGE_BACKEND>` and :ref:`CONFIG_NCS_MATTER_SECURE_STORAGE_BACKEND<CONFIG_NCS_MATTER_SECURE_STORAGE_BACKEND>` enabled), all methods of the generic interface are supported.
...
* :ref:`CONFIG_NCS_MATTER_SECURE_STORAGE_MAX_ENTRY_NUMBER<CONFIG_NCS_MATTER_SECURE_STORAGE_MAX_ENTRY_NUMBER>` - Defines the maximum number or assets that can be stored in the secure storage.
...
To keep the crash log in the memory after reading it, set the :ref:`CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS_REMOVE_CRASH_AFTER_READ<CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS_REMOVE_CRASH_AFTER_READ>` Kconfig option to ``n``.
...
Because of this, for the network and the end-user logs the :ref:`CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS_REDIRECT<CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS_REDIRECT>` Kconfig option is enabled by default.
...
You can disable the redirect functionality by disabling the :ref:`CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS_REDIRECT<CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS_REDIRECT>` Kconfig option.
...
The snippet sets the following Kconfig options:
  ...
  * :ref:`CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS<CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS>` to ``y``.
  * :ref:`CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS_CRASH_LOGS<CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS_CRASH_LOGS>` to ``y``.
  * :ref:`CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS_REMOVE_CRASH_AFTER_READ<CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS_REMOVE_CRASH_AFTER_READ>` to ``y``.
  * :ref:`CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS_END_USER_LOGS<CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS_END_USER_LOGS>` to ``y``.
  * :ref:`CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS_NETWORK_LOGS<CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS_NETWORK_LOGS>` to ``y``.
  * :kconfig:option:`CONFIG_LOG_MODE_DEFERRED` to ``y``.
  * :kconfig:option:`CONFIG_LOG_RUNTIME_FILTERING` to ``n``.

Deferred logs mode (:kconfig:option:`CONFIG_LOG_MODE_DEFERRED`) is enabled because it is required by the log redirection functionality (:ref:`CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS_REDIRECT<CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS_REDIRECT>`), which is enabled by default for diagnostic network and end-user logs.

.. note::

  You cannot set the :ref:`CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS<CONFIG_NCS_MATTER_DIAGNOSTIC_LOGS>` Kconfig option separately without adding the devicetree overlays contained in the snippet.
