...
#ifdef CONFIG_NCS_MATTER_WATCHDOG_PAUSE_IN_SLEEP
	watchdogOptions |= WDT_OPT_PAUSE_IN_SLEEP;
#endif /* CONFIG_NCS_MATTER_WATCHDOG_PAUSE_IN_SLEEP */
#ifdef CONFIG_NCS_MATTER_WATCHDOG_PAUSE_ON_DEBUG
	watchdogOptions |= WDT_OPT_PAUSE_HALTED_BY_DBG;
#endif /* CONFIG_NCS_MATTER_WATCHDOG_PAUSE_ON_DEBUG */

	if (wdt_setup(wdt, watchdogOptions) != 0) {
		return false;
	}

	LOG_DBG("Global watchdog started with timeout %d ms", CONFIG_NCS_MATTER_WATCHDOG_TIMEOUT);
	sEnabled = true;
	return true;
}
...
bool WatchdogSource::Install()
{
	...
	wdtConfig.window.max = CONFIG_NCS_MATTER_WATCHDOG_TIMEOUT;
