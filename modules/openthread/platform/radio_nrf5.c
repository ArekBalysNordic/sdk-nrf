...
static otError transmit_frame(otInstance *aInstance)
{
	...
	LOG_INF("RADIO: Checking HDR - mIsHdrFrame=%d, length=%u, threshold=127",
		nrf5_data.tx.frame.mIsHdrFrame, nrf5_data.tx.frame.mLength);

	if (nrf5_data.tx.frame.mIsHdrFrame && nrf5_data.tx.frame.mLength > 127) {
		LOG_INF("RADIO: HDR frame confirmed! Starting HDR transmission (%u bytes, state=%d)",
